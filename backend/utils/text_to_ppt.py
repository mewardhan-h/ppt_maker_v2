...
def set_bullet_format(p):
    ...
    # Hanging indent: left margin 0.38", first-line indent -0.38"
    pPr.set('marL', str(int(Inches(0.38))))
    pPr.set('indent', str(-int(Inches(0.38))))

    # Suppress any inherited automatic bullet so only our text bullet shows
...
def add_logo_left(slide, logo_path: str):
    """Add logo image to the top-left corner of a slide."""
    slide.shapes.add_picture(logo_path, LOGO_LEFT_LEFT, LOGO_LEFT_TOP, width=LOGO_LEFT_WIDTH, height=LOGO_LEFT_HEIGHT)


def add_logo_right(slide, logo_path: str):
    """Add logo image to the top-right corner of a slide."""
    slide.shapes.add_picture(logo_path, LOGO_RIGHT_LEFT, LOGO_RIGHT_TOP, width=LOGO_RIGHT_WIDTH, height=LOGO_RIGHT_HEIGHT)
...
def add_title_slide(prs, topic: str, presenter: str, department: str,
    ...
    # --- Presenter info text box (bottom-left) ---
    ...
    # Logos
    if logo1_path and os.path.isfile(logo1_path):
        add_logo_left(slide, logo1_path)
    if logo2_path and os.path.isfile(logo2_path):
        add_logo_right(slide, logo2_path)


def add_header_topic(slide, title: str):
    """Add slide title centered (horizontal + vertical) between the two logos."""
...
def create_ppt(slides: list[dict], output_path: str, logo1_path: str = None,
               logo2_path: str = None, presenter: str = "", department: str = ""):
    prs = Presentation()
    ...
    # --- Title slide (slide 0) ---
    add_title_slide(prs, slides[0]["title"], presenter, department, logo1_path, logo2_path)
    # Content box dimensions and margins
    ...
    for slide_data in slides[1:]:
        ...
        # Remove built-in placeholders entirely so no "Click to add..." prompts appear
        for ph in list(slide.placeholders):
            ph._element.getparent().remove(ph._element)

        # Custom content: bullets and/or table
        ...
        if slide_data.get("table"):
            ...
            # If bullets exist, shift table below them
        ...
        # Add logos to every slide
        if logo1_path and os.path.isfile(logo1_path):
            add_logo_left(slide, logo1_path)
        if logo2_path and os.path.isfile(logo2_path):
            add_logo_right(slide, logo2_path)

        # Add slide title as header between logos
...
def main():
    ...
    # Default file paths
    input_file = "basic_text_converted.txt"
    output_file = "presentation.pptx"
    logo1_path = "assets/logo_left.jpg"
    logo2_path = "assets/logo_right.jpg"
    
    # Check if input file exists
    if not os.path.isfile(input_file):
        print(f"❌ Error: Input file '{input_file}' not found")
        print("Make sure basic_text_converted.txt exists in the current directory")
        print("\nTip: Run 'python llm_formatter.py raw_input.txt' first to create it")
        sys.exit(1)
    
    # Check logos
    if not os.path.isfile(logo1_path):
        print(f"⚠️  Warning: Left logo not found: {logo1_path}")
        logo1_path = None
    else:
        print(f"✅ Found left logo: {logo1_path}")
    
    if not os.path.isfile(logo2_path):
        print(f"⚠️  Warning: Right logo not found: {logo2_path}")
        logo2_path = None
    else:
        print(f"✅ Found right logo: {logo2_path}")
    ...
    # Update first slide title with the entered topic
    updated = re.sub(r'^#\s+.+', f'# {topic}', original, count=1, flags=re.MULTILINE)
    
    # Parse slides
    slides = parse_text(updated)
    if not slides:
        print("❌ No slides found in the input file")
        sys.exit(1)
    
    print(f"🎨 Generating {len(slides)} slide(s)...")
    create_ppt(slides, output_file, logo1_path=logo1_path, logo2_path=logo2_path,
               presenter=presenter, department=department)
    
    print("\n" + "="*60)
    print(f"🎉 SUCCESS! PowerPoint created: {output_file}")
    print("="*60 + "\n")
